  ...
  return (
    <>
      <div
        ...
        style={{ background: WH, borderRadius: 24, boxShadow: '0 6px 24px rgba(26,92,53,0.08)', border: `1px solid ${L}`, overflow: 'hidden', cursor: 'pointer' }}
...
<div
  style={{
    margin: isOwner ? '8px 12px 0' : '12px 12px 0',
    borderRadius: 20,
    padding: '16px',
minHeight: 170,
  ...
  <div style={{ display: 'flex', gap: 14, alignItems: 'stretch' }}>
    <div style={{ flex: 1, minWidth: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 10 }}>
        <span
          style={{
            background: isClosed ? '#FEE2E2' : isClosingSoon ? 'rgba(245,158,11,0.12)' : L,
            color: isClosed ? '#DC2626' : isClosingSoon ? '#B45309' : P,
            borderRadius: 999,
            padding: '4px 9px',
            fontSize: 11,
            fontWeight: 800,
            border: `1px solid ${isClosed ? 'rgba(220,38,38,0.16)' : isClosingSoon ? 'rgba(245,158,11,0.22)' : 'rgba(108,191,108,0.28)'}`,
          }}
        >
          {isClosed ? '모집 마감' : isClosingSoon ? '마감 임박' : '모집중'}
        </span>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
        {fmtCourtNum((court as { court_number?: string }).court_number) && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            <MapPin style={{ width: 12, height: 12, color: A, flexShrink: 0 }} />
            <span style={{ color: P2, fontSize: 12, fontWeight: 700 }}>
              {fmtCourtNum((court as { court_number?: string }).court_number)}
            </span>
          </div>
        )}
        ...
        {(court.date || court.start_time) && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            <Calendar style={{ width: 12, height: 12, color: A, flexShrink: 0 }} />
            <span style={{ color: M, fontSize: 12, fontWeight: 700 }}>
        ...
        {court.court_fee != null && court.court_fee >= 0 && (
          <div style={{ color: G, fontSize: 12, fontWeight: 800, marginTop: 2 }}>
            매칭비 {fmtFee(court.court_fee)}
          </div>
        )}
      </div>
...
minWidth: 96,
        borderRadius: 18,
        overflow: 'hidden',
        background: `linear-gradient(135deg, ${L}, #FFFFFF)`,
        border: `1px solid rgba(26,92,53,0.10)`,
        display: 'flex',
        alignItems: 'center',
