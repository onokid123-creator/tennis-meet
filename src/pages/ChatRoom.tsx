...
const { user, getSafeUser } = useAuth();
const roomState = location.state as {
  from?: 'tennis' | 'dating';
  initialOtherUser?: Profile | null;
  initialCourtName?: string | null;
  initialPurpose?: string | null;
} | null;

const fromTab = roomState?.from ?? 'tennis';
useEffect(() => {
  ...
  const [otherUser, setOtherUser] = useState<Profile | null>(roomState?.initialOtherUser ?? null);
  const [chatPurpose, setChatPurpose] = useState<string | null>(roomState?.initialPurpose ?? fromTab);
...
// 채널이 이미 마운트된 상태인지 추적 (중복 구독 방지)
const channelMountedRef = useRef(false);

useEffect(() => {
  const handleAuthResynced = () => {
    console.log('[ChatRoom] auth-resynced → 데이터 재로딩');
...
if (window.location.pathname.startsWith('/chat/')) {
  navigate('/chats', { state: { from: fromTab } });
  return;
}
...
const getCurrentUser = useCallback(async () => {
  // getSession()은 resume 후 stale → getSafeUser 사용
...
const scrollToBottomAfterKeyboard = useCallback(() => {
  ...
  // ── 키보드 처리 (카카오톡 방식) ──────────────────────
  // @capacitor/keyboard 플러그인의 resize:'native' 설정과 함께 동작.
  // 키보드가 올라오면 WebView 자체가 줄어들어서 입력창이 키보드 바로
  // 위에 붙는다. keyboardDidShow 시점에 스크롤을 맨 아래로 내려서
  // 최근 메시지가 키보드에 가려지지 않게 한다.
  useEffect(() => {
...
useEffect(() => {
  ...
  const loadRoomInfo = async (attempt = 0) => {
    if (cancelled) return;
    const currentUser = await getSafeUser();
    if (cancelled) return;

    if (!currentUser) {
      // 최대 3번까지만 재시도 (이전 6번 → resume 후 너무 오래 걸림)
      if (attempt < 3) {
    ...
    participants.forEach((p) => {
      confirmedMap[p.user_id] = p.is_confirmed ?? false;
      initLastReads[p.user_id] = p.last_read_at ?? null;
    });
    ...
    if (chat?.court_id) {
      ...
      supabase
        .from('courts')
        .select('court_name, date, start_time, end_time, court_number')
        .eq('id', chat.court_id)
        .maybeSingle()
        .then(({ data }) => {
          if (cancelled) return;
          ...
          if (data) {
            setCourtInfo({
              ...
              court_number: data.court_number ?? undefined,
            });
          }
        });
    }
    ...
    const rawConfirmedIds: string[] = chat?.confirmed_user_ids ?? [];
    const chatConfirmedIds = isGroup
      ? rawConfirmedIds
      : rawConfirmedIds.filter((id) => id !== currentUser.id);
    ...
    if (chatConfirmedIds.length > 0) {
      ...
      if (!cancelled && confirmedProfs) {
        setConfirmedParticipants(
          confirmedProfs.map((p) => ({
            user_id: p.user_id,
            name: p.name,
            photo_url: p.photo_url ?? undefined,
            tennis_photo_url: p.tennis_photo_url ?? undefined,
    ...
    const oppFromParticipants = participants.find(
    ...
    if (oppPartRes.data?.last_read_at) {
      setOtherLastRead(oppPartRes.data.last_read_at);
    }
  };
...
useEffect(() => {
  ...
  const load = async () => {
    ...
    if (!currentUser) {
      // 유저를 못 가져옴 = 세션 연결이 죽음 → reload (1회만)
      console.warn('[ChatRoom] currentUser 없음');
      const reloadKey = 'chatroom_reload_user_' + chatId;
      if (!sessionStorage.getItem(reloadKey) && Capacitor.isNativePlatform()) {
    ...
    sessionStorage.removeItem('chatroom_reload_user_' + chatId);
    ...
    try {
      ...
      // 3) 메시지가 보이도록 먼저 로딩 해제 (프로필은 뒤에서 채움)
      //    이전: 프로필 조회까지 끝나야 로딩 해제 → 느림
      setLoading(false);

      // 4) sender 프로필은 한 번에 묶어서 조회 (N+1 아님 - 이미 .in() 사용)
      ...
      console.error('[ChatRoom] load 예외:', err);
...
// resume(resumeTick 증가) 시 채널을 깔끔하게 재생성한다.
// AppLifecycleBridge가 realtime.disconnect/connect를 하면 기존 채널이
// CLOSED 상태로 죽어버리는데, 같은 채널명으로는 재구독이 잘 안 됨.
// → resumeTick을 channelKey에 넣어서 매번 새 채널명으로 재구독
//   (이전 채널은 이 useEffect의 cleanup에서 removeChannel로 제거됨)
const channelKey = `${chatId}_${user.id}_${resumeTick}`;

    const msgChannel = supabase
      .channel(`room_messages_${channelKey}`)
      .on(
        'postgres_changes',
  ...
  if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT' || status === 'CLOSED') {
    console.warn('[ChatRoom] msgChannel 상태:', status);
  }
}); 

    const partChannel = supabase
      .channel(`room_participants_${channelKey}`)
      .on(
        'postgres_changes',
  ...
  navigate('/chats', { replace: true, state: { from: fromTab } });
}
 else {
  ...
  }, [chatId, user, refreshParticipantCount, navigate, resumeTick]);
...
const sendMessage = async (content: string, type: string = 'user', extraPayload?: Record<string, unknown>) => {
  ...
  try {
    ...
    if (otherIds.length > 0) {
     const { data: rawTargetProfiles } = await supabase
  .from('profiles')
  .select('user_id, fcm_token, name')
  .in('user_id', otherIds);

const targetProfiles = Array.from(
  new Map((rawTargetProfiles ?? []).map((p) => [p.user_id, p])).values()
);
      console.log('[PUSH] targetProfiles:', JSON.stringify(targetProfiles));
const { data: senderProfile } = await supabase
  .from('profiles')
  .select('name')
  .eq('user_id', currentUser.id)
  .maybeSingle();
      const title = senderProfile?.name || '누군가';
      const body = trimmed.length > 60 ? `${trimmed.slice(0, 60)}...` : trimmed;
...
console.log('[PUSH] send-push result:', JSON.stringify({
 ...
 const handleAfterResponse = async (messageId: string, accept: boolean) => {
  ...
  await supabase
    .from('messages')
    .update({ payload: { status: accept ? 'accepted' : 'declined' } })
    .eq('id', messageId);
  ...
  navigate(`/chat/${newChat.id}`, { replace: true, state: { from: fromTab } });
}; 
 ...
 const loadPendingMealProposals = async () => {
  ...
  const handleSendMealProposal = async (receiverId: string) => {
    if (!user || !chatId || mealProposalSubmitting) return;
...
const { data: senderProfile } = await supabase
  .from('profiles')
  .select('fcm_token')
  .eq('user_id', proposalData?.sender_id)
  .maybeSingle();

if (senderProfile?.fcm_token) {
  await supabase.functions.invoke('send-push', {
    body: {
      token: senderProfile.fcm_token,
      title: receiverProfile?.name || '상대방',
      body: '식사 제안이 수락됐어요 😊',
      data: {
...
if (otherUser && user) {
  const targetUserId = otherUser.user_id || otherUser.id;
...
const targetUserId =
  otherUser?.user_id || otherUser?.id;
...
if (receiverProfile?.fcm_token) {
  ...
  const { data: mealReceiverProfile } = await supabase
    .from('profiles')
    .select('fcm_token')
    .eq('user_id', participantId)
    .maybeSingle();

  if (mealReceiverProfile?.fcm_token) {
    await supabase.functions.invoke('send-push', {
      body: {
...
paddingBottom: '14px',
      ...
      >
     <button
  onClick={() => navigate('/chats', { state: { from: fromTab } })}
      ...
      {showParticipantMgmt && isGroupChat && (
        ...
        >
          ...
          <div className="px-3 pb-2.5 flex flex-col gap-1.5">
           ...
           const avDropPhoto = avDropBlocked ? null : (isDating ? av.photo_url : av.tennis_photo_url);
...
/>
  ...
  <img src={avPhoto} alt={av.name} className="w-full h-full object-cover" />
               ...
               const avPhoto = isDating ? otherUser.photo_url : otherUser.tennis_photo_url;
