...
export default function CreateCourt() {
  ...
  const [purpose] = useState<CategoryTab>(() => {
  if (isEditing) return editCourt!.purpose as CategoryTab;
  if (passedPurpose === 'tennis' || passedPurpose === 'dating') return passedPurpose;
  if (profile?.purpose === 'tennis' || profile?.purpose === 'dating') return profile.purpose as CategoryTab;
  return getActiveCategoryPurpose();
});
...
delete_at: deleteAt.toISOString(),
  ...
  return (
 <div className="h-[100dvh] bg-[#F8F9F4] flex flex-col overflow-hidden">
      ...
      {/* Header */}
     <header
  className="bg-white border-b border-gray-200 px-4 pb-3 flex items-center gap-3 flex-shrink-0 relative z-50"
 style={{ paddingTop: 'calc(env(safe-area-inset-top) + 12px)' }}
>
       <button onClick={goBack} className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-gray-100 transition relative z-50">
   ...
   <div className="px-5 pt-4 pb-2 flex-shrink-0">
      ...
      {/* Content */}
     <div className="flex-1 overflow-y-auto px-5 pt-5 pb-36" ref={slideRef} style={slideStyle}>
        {step === 1 && (
          <CourtMapSearch
      ...
      {/* Bottom button */}
     <div
  className="fixed left-0 right-0 bottom-0 z-40 px-5 bg-white border-t border-gray-100 pt-3"
