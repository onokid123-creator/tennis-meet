...
export default function PurposeSelection() {
  ...
  is_subscribed: false,
  free_meeting_count: 0,
})        .eq('user_id', user.id);
      await refreshProfile();
localStorage.setItem('home_category_tab', selectedPurpose);
localStorage.setItem('home_active_tab', 'others');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#1B4332] to-[#2d5a4a] flex flex-col items-center justify-center px-6 py-12">
      <h2 className="text-white text-2xl font-semibold tracking-tight text-center leading-snug mb-10">
  테니스 스타일을 선택해주세요
</h2>
...
>
  ...
  <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end p-6">
    <div className="text-white">
      ...
      <p className="text-sm opacity-90">테니스에 집중한 매칭</p>
    </div>
  </div>
</button>
        <button
