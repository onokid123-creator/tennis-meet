...
useEffect(() => {
  const from = (location.state as { from?: 'tennis' | 'dating' } | null)?.from;
...
const directChatIds = chatsData.map((chat) => chat.id);

const [allMessagesRes, unreadMessagesRes, allGroupPartsRes] = await Promise.all([
  supabase
    .from('messages')
    .select('id, chat_id, content, created_at, sender_id, type')
...
(unreadMessagesRes.data ?? []).forEach((msg) => {
  unreadCountMap[msg.chat_id] = (unreadCountMap[msg.chat_id] ?? 0) + 1;
});

const allGroupParts = allGroupPartsRes.data ?? [];
const allGroupUserIds = Array.from(new Set(allGroupParts.map((p) => p.user_id)));

const groupProfileMap: Record<string, any> = {};
if (allGroupUserIds.length > 0) {
  const { data: allGroupProfiles } = await supabase
    .from('profiles')
    .select('user_id, name, photo_url, tennis_photo_url')
    .in('user_id', allGroupUserIds);

  (allGroupProfiles ?? []).forEach((p) => {
    groupProfileMap[p.user_id] = p;
  });
}

const groupTitleMap: Record<string, string> = {};
const groupParticipantPhotosMap: Record<string, Array<{ user_id: string; name: string; photo_url?: string; tennis_photo_url?: string }>> = {};

chatsData
  .filter((chat) => chat.is_group)
  .forEach((chat) => {
    const parts = allGroupParts.filter((p) => p.chat_id === chat.id);
    const profs = parts.map((p) => groupProfileMap[p.user_id]).filter(Boolean);
    const others = profs.filter((p) => p.user_id !== currentUser.id);
    const names = others.map((p) => p.name);

    if (names.length > 0) {
      groupTitleMap[chat.id] = buildGroupTitle(names);
    }

    groupParticipantPhotosMap[chat.id] = profs.map((p) => ({
      user_id: p.user_id,
      name: p.name,
...
const chatRows = chatsData.map((chat) => {
  ...
  return {
    ...
    groupParticipantPhotos: groupParticipantPhotosMap[chat.id],
          ...
          const chatsWithMessages = chatRows.map(({ chat, opponentId, lastMsg, unreadCount, groupTitle, groupParticipantPhotos }) => {
            ...
            return {
              ...chat,
             user1: !chat.is_group && (chat.user1_id as string) !== currentUser.id ? opponentProfile : null,
            user2: !chat.is_group && chat.user2_id && (chat.user2_id as string) !== currentUser.id ? opponentProfile : null,
  ...
  sessionStorage.setItem(`cached_chats_${currentUser.id}`, JSON.stringify(chatsWithMessages));
}
  ...
  if (!hasLoadedOnceRef.current) {
    setChats([]);
     ...
     const { data: groupParticipantRows, error: groupParticipantRowsError } = await supabase
...
const cachedChats = sessionStorage.getItem(`cached_chats_${user.id}`);
const cachedGroupChats = sessionStorage.getItem(`cached_group_chats_${user.id}`);
    ...
    if (cachedGroupChats) {
      setGroupChats(JSON.parse(cachedGroupChats));
      hasLoadedOnceRef.current = true;
      setLoading(false);
    }
  } catch (e) {
    console.warn('[Chats] cache restore error:', e);
  }

  // 2) user가 준비됐을 때만 fetch 실행
if (user) {
  const hasCache =
    !!sessionStorage.getItem(`cached_chats_${user.id}`) ||
    !!sessionStorage.getItem(`cached_group_chats_${user.id}`);
  fetchChats({ silent: hasCache });
}
  // 3) realtime은 user 있을 때만 연결
  if (!user) return;
...
let realtimeSetupTimer: ReturnType<typeof setTimeout> | null = null;

const removeRealtimeChannels = () => {
  if (channel) {
    console.log('[Chats] 기존 channel 제거');
    supabase.removeChannel(channel);
    channel = null;
  }

  if (kickChannel) {
    console.log('[Chats] 기존 kickChannel 제거');
    supabase.removeChannel(kickChannel);
    kickChannel = null;
  }
};

const setupRealtime = (reason: string) => {
  console.log('[Chats] realtime 새로 생성:', reason);
  ...
  realtimeSetupTimer = setTimeout(() => {
    const uniqueKey = `${user.id}_${Date.now()}`;

    channel = supabase
      .channel(`chats_list_${uniqueKey}`)
    ...
    kickChannel = supabase
      .channel(`global_kick_${uniqueKey}`)
      ...
      .subscribe((status) => {
        console.log('[Chats] kickChannel status:', status);
      });
  }, 700);
};
...
const tennisDirectChats = chats.filter((c) => c.purpose === 'tennis' || !c.purpose);
...
if (tab === 'tennis') {
  ...
  return (
    ...
    >
     ...
     {!hasLoadedOnce && chats.length === 0 && groupChats.length === 0 ? (
            ...
            <div className="space-y-2">
              {tennisGroupChats.map((gc) => (
                <TennisGroupChatRow
                  ...
                  onPress={() => navigate(`/group-chat/${gc.id}`, { state: { from: 'tennis' } })}
                />
              ))}
  ...
  navigate(`/chat/${chat.id}`, {
    state: {
      ...
      initialCourtName: extended.court_name,
      initialPurpose: chat.purpose ?? 'tennis',
    },
  })
}
                  />
          ...
          <div className="space-y-2.5">
            {datingGroupChats.map((gc) => (
              <DatingGroupChatRow
                ...
                gc={gc}
                blockedIds={blockedIds}
               onPress={() => navigate(`/group-chat/${gc.id}`, { state: { from: 'dating' } })}
              />
            ))}
  ...
  navigate(`/chat/${chat.id}`, {
    state: {
      ...
      initialOtherUser: other,
      initialCourtName: extended.court_name,
      initialPurpose: chat.purpose ?? 'dating',
    },
  })
}
                />
...
const hostPhoto = isHostBlocked ? undefined : host?.tennis_photo_url;
...
function GroupAvatarGrid({
  ...
  const getPhoto = (p: { photo_url?: string; tennis_photo_url?: string; _blocked?: boolean }) => {
    ...
    return isTennis ? p.tennis_photo_url : p.photo_url;
  };
